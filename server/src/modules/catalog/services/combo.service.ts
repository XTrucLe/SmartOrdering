...
export class ComboService {
    ...
    async updateCombo(storeId: string, comboId: string, updateComboDto: UpdateComboDto): Promise<Combo> {
    ...
    async handlePurchase(storeId: string, comboId: string, quantity: number): Promise<void> {
        ...
        await this.dataSource.transaction(async (manager) => {
            const currentCombo = await manager.findOne(Combo, { where: { id: combo.id }, lock: { mode: 'pessimistic_write' } });
            ...
            const result = await manager.createQueryBuilder()
                .update(Combo)
                .set({ soldQty: () => `"sold_qty" + ${quantity}`, isActive: () => `CASE WHEN quantity_limit != -1 AND sold_qty + ${quantity} >= quantity_limit THEN false ELSE is_active END` })
                .where('id = :id', { id: combo.id })
                .andWhere('quantity_limit = -1 OR sold_qty + :quantity <= quantity_limit', { quantity })
                .execute();

            if (result.affected === 0) {
                throw new UnprocessableEntityException(`Cannot purchase Combo ${comboId} in store ${storeId}, possibly due to quantity exceeding limit or Combo being sold out.`);
            }
        });
    }
    ...
    async deleteCombo(storeId: string, comboId: string): Promise<void> {
        ...
        await this.dataSource.transaction(async (manager) => {
            await manager.delete(ComboItem, { comboId: combo.id });
            const result = await manager.delete(Combo, { id: combo.id, storeId, isActive: true });
