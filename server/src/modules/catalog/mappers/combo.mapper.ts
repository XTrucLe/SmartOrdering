...
export class ComboMapper {
  static toComboDto(combo: Combo): ComboDto {
    const comboDto = plainToInstance(ComboDto, combo, {
      excludeExtraneousValues: true,
    });
    if (combo.comboItems) {
      comboDto.comboItems = combo.comboItems.map((item) =>
        plainToInstance(ComboItemDto, item, { excludeExtraneousValues: true }),
      );
    }
    return comboDto;
  }
  ...
  static toComboItemDto(comboItem: ComboItem): ComboItemDto {
    return plainToInstance(ComboItemDto, comboItem, {
      excludeExtraneousValues: true,
    });
  }
