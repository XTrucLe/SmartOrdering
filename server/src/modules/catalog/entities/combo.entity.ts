...
import { Entity, PrimaryGeneratedColumn, Column, ManyToOne, Check, CreateDateColumn, OneToMany } from 'typeorm';
...
export class Combo {
    ...
    isValidNow(): boolean {
        const now = new Date();
        return this.isActive && (!this.validFrom || this.validFrom <= now)
            && (!this.validTo || this.validTo >= now) && !this.isSoldOut();
    }
}
